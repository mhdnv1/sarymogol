...
const Faqs = () => {
    ...
    const getApi = () => {
        axios.get(`https://sarymogol.com/api/questionsget`)
            .then(({ data }) =>
            {
                console.log(data) 
                setUser(data)
                        ...
                        return (
                            <Accordion defaultActiveKey="0">
                                <Accordion.Item eventKey="0">
                                    <Accordion.Header>{langContext.bool ? item.questionRus:item.questionEng}</Accordion.Header>
                                    <Accordion.Body>
                                        {langContext.bool ? item.answerRus:item.answerEng}
                                    </Accordion.Body>
                                </Accordion.Item>
                            </Accordion>
                        )
